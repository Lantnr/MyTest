...
namespace TGG.Module.RoleTrain
{
    ...
    public class Handle
    {
        ...
        {
#if DEBUG
            XTrace.WriteLine("进入修行模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("进入修行模块 当前计数{0}", count));

            _token = new CancellationTokenSource();
            var _module = CommonHelper.GetCDTSTM(this.GetType().Namespace, count, true);
            ConcurrentQueueExtensions.Clear(Variable.GMQ_ROLETRAIN);
            try
            {
                Task.Factory.StartNew(m =>
                {
                    ...
                    {
                      if (Variable.GMQ_ROLETRAIN.Count > 0)
                      {
                          dynamic data;
                          Variable.GMQ_ROLETRAIN.TryDequeue(out data);
        ...
        {
            DisplayGlobal.log.Write(string.Format("退出修行模块 当前计数{0}", count));
