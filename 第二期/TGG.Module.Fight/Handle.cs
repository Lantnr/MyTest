...
namespace TGG.Module.Fight
{
    ...
    public class Handle
    {
        ...
        {
#if DEBUG
            XTrace.WriteLine("进入战斗模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("进入战斗模块 当前计数{0}", count));

            _token = new CancellationTokenSource();
            var _module = CommonHelper.GetCDTSTM(this.GetType().Namespace, count, true);
            ConcurrentQueueExtensions.Clear(Variable.GMQ_FIGHT);
            try
            {
                Task.Factory.StartNew(m =>
                {
                    ...
                    {
                        if (Variable.GMQ_FIGHT.Count > 0)
                        {
                            dynamic data;
                            Variable.GMQ_FIGHT.TryDequeue(out data);
        ...
        {
#if DEBUG
            XTrace.WriteLine("退出战斗模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("退出战斗模块 当前计数{0}", count));
