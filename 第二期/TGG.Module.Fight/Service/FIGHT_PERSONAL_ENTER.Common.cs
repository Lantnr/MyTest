...
namespace TGG.Module.Fight.Service
{
    ...
    public partial class FIGHT_PERSONAL_ENTER
    {
        ...
        public ResultType FightMethod(TGGSession session, Int64 hp)
        {
            //start
        ...
        /// <returns></returns>
        private ResultType ReadyData(TGGSession session, Int64 hp = 0)
        {
            Init();
            var rivalid = session.Fight.Rival;
            ...
            VocationAdd(session);
            var oneself = ConvertFightPersonal(session.Fight.Personal);          //己方阵形
            oneself = IsUpdatePersonal(session, oneself);                        //验证是否为只允许主角上阵
        ...
        private void VocationAdd(TGGSession session)
        {
            var userid = session.Player.User.id;
            var rivalid = session.Fight.Rival;
        ...
        {
            var list = list_buff.Where(m => m.round <= Round).ToList();
            //list_att.RemoveAll(m => m.round == Round);
        ...
        private void IsUpdateHp(TGGSession session, Int64 hp)
        {
            if (hp <= 0) return;
            ...
            switch (RivalFightType)
            {
                ...
                case (int)FightType.BUILDING:
                    {
                        fightrole = list_role.FirstOrDefault(m => m != null && m.user_id != user.id);
                        _fightrole = list_role_hp.FirstOrDefault(m => m != null && m.user_id != user.id);
                        break;
                    }
                case (int)FightType.CONTINUOUS:
                case (int)FightType.DUPLICATE_SHARP:
                    {
                        fightrole = list_role.FirstOrDefault(m => m.id == role.id);
                        _fightrole = list_role_hp.FirstOrDefault(m => m.id == role.id);
                        break;
                    }
                default: { return; }
            }
...
#endif
            (new Share.Title()).RoleFightMethods(userid);
#if DEBUG
            sw.Stop();
