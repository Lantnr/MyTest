...
namespace TGG.Module.Role
{
    ...
    public class Handle
    {
        ...
        {
#if DEBUG
            XTrace.WriteLine("进入武将模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("进入武将模块 当前计数{0}", count));

            _token = new CancellationTokenSource();
            var _module = CommonHelper.GetCDTSTM(this.GetType().Namespace, count, true);
            ConcurrentQueueExtensions.Clear(Variable.GMQ_ROLE);
            try
            {
                Task.Factory.StartNew(m =>
                {
                    ...
                    {
                        if (Variable.GMQ_ROLE.Count > 0)
                        {
                            dynamic data;
                            Variable.GMQ_ROLE.TryDequeue(out data);
        ...
        {
#if DEBUG
            XTrace.WriteLine("退出武将模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("退出武将模块 当前计数{0}", count));
