...
namespace TGG.Module.Consume
{
    ...
    /// 开发者：李德雁
    /// </summary>
    public class TASK_VOCATION_BUY : IConsume
    {
        public ASObject Execute(Int64 userid, ASObject data)
        {
            ...
            if (type != 1 && type != 2) return new Share.TGTask().BuildVoctionData((int)ResultType.FRONT_DATA_ERROR, null, 0);
            ...
            if (!CheckIdentify(voc, identify))
                return new Share.TGTask().BuildVoctionData((int)ResultType.TASK_VOCATION_NONEW, null, 0); ;//大名没有职业任务
            var newtasks = new Share.TGTask().GetNewVocationTasks(identify, userid, voc);
            if (!newtasks.Any()) return new Share.TGTask().BuildVoctionData((int)ResultType.TASK_VOCATION_NOREFLASH, null, 0);
