...
namespace TGG.Module.Equip
{
    ...
    public class Handle
    {
        ...
        {
#if DEBUG
            XTrace.WriteLine("进入装备模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("进入装备模块 当前计数{0}", count));

            _token = new CancellationTokenSource();
            var _module = CommonHelper.GetCDTSTM(this.GetType().Namespace, count, true);
            ConcurrentQueueExtensions.Clear(Variable.GMQ_EQUIP);
            try
            {
                Task.Factory.StartNew(m =>
                {
                    ...
                    {
                        if (Variable.GMQ_EQUIP.Count > 0)
                        {
                            dynamic data;
                            Variable.GMQ_EQUIP.TryDequeue(out data);
        ...
        {
#if DEBUG
            XTrace.WriteLine("退出装备模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("退出装备模块 当前计数{0}", count));
