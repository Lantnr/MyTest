...
namespace TGG.Module.Skill
{
    ...
    public class Handle
    {
        ...
        {
#if DEBUG
            XTrace.WriteLine("进入技能模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("进入技能模块 当前计数{0}", count));

            _token = new CancellationTokenSource();
            var _module = CommonHelper.GetCDTSTM(this.GetType().Namespace, count, true);
            ConcurrentQueueExtensions.Clear(Variable.GMQ_SKILL);
            try
            {
                Task.Factory.StartNew(m =>
                {
                    ...
                    {
                        if (Variable.GMQ_SKILL.Count > 0)
                        {
                            dynamic data;
                            Variable.GMQ_SKILL.TryDequeue(out data);
        ...
        {
#if DEBUG
            XTrace.WriteLine("退出技能模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("退出技能模块 当前计数{0}", count));
