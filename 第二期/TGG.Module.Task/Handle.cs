...
#endif
            DisplayGlobal.log.Write(string.Format("进入任务模块 当前计数{0}", count));

            _token = new CancellationTokenSource();
            var _module = CommonHelper.GetCDTSTM(this.GetType().Namespace, count, true);
            ConcurrentQueueExtensions.Clear(Variable.GMQ_TASK);
            try
            {
                System.Threading.Tasks.Task.Factory.StartNew(m =>
                {
                    ...
                    {
                        if (Variable.GMQ_TASK.Count > 0)
                        {
                            dynamic data;
                            Variable.GMQ_TASK.TryDequeue(out data);
