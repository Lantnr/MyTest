...
namespace TGG.Module.Appraise
{
    ...
    public class Handle
    {
        ...
        {
#if DEBUG
            XTrace.WriteLine("进入评定模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("进入评定模块 当前计数{0}", count));

            _token = new CancellationTokenSource();
            var _module = CommonHelper.GetCDTSTM(this.GetType().Namespace, count, true);
            ConcurrentQueueExtensions.Clear(Variable.GMQ_APPRAISE);
            try
            {
                Task.Factory.StartNew(m =>
                {
                    ...
                    {
                        if (Variable.GMQ_APPRAISE.Count > 0)
                        {
                            dynamic data;
                            Variable.GMQ_APPRAISE.TryDequeue(out data);
        ...
        {
#if DEBUG
            XTrace.WriteLine("退出评定模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("退出评定模块 当前计数{0}", count));
