...
namespace TGG.Module.Family
{
    ...
    public class Handle
    {
        ...
        {
#if DEBUG
            XTrace.WriteLine("进入家族模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("进入家族模块 当前计数{0}", count));

            _token = new CancellationTokenSource();
            var _module = CommonHelper.GetCDTSTM(this.GetType().Namespace, count, true);
            ConcurrentQueueExtensions.Clear(Variable.GMQ_FAMILY);
            try
            {
                Task.Factory.StartNew(m =>
                {
                    ...
                    {
                        if (Variable.GMQ_FAMILY.Count > 0)
                        {
                            dynamic data;
                            Variable.GMQ_FAMILY.TryDequeue(out data);
        ...
        {
#if DEBUG
            XTrace.WriteLine("退出家族模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("退出家族模块 当前计数{0}", count));
