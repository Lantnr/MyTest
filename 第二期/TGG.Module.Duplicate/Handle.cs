...
namespace TGG.Module.Duplicate
{
    ...
    public class Handle
    {
        ...
        {
#if DEBUG
            XTrace.WriteLine("进入副本模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("进入副本模块 当前计数{0}", count));

            _token = new CancellationTokenSource();
            var _module = CommonHelper.GetCDTSTM(this.GetType().Namespace, count, true);
            ConcurrentQueueExtensions.Clear(Variable.GMQ_DUPLICATE);
            try
            {
                Task.Factory.StartNew(m =>
                {
                    ...
                    {
                        if (Variable.GMQ_DUPLICATE.Count > 0)
                        {
                            dynamic data;
                            Variable.GMQ_DUPLICATE.TryDequeue(out data);
        ...
        {
#if DEBUG
            XTrace.WriteLine("退出副本模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("退出副本模块 当前计数{0}", count));
