...
namespace TGG.Module.Business
{
    ...
    public class Handle
    {
        ...
        {
#if DEBUG
            XTrace.WriteLine("进入跑商模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("进入跑商模块 当前计数{0}", count));

            _token = new CancellationTokenSource();
            var _module = CommonHelper.GetCDTSTM(this.GetType().Namespace, count, true);
            ConcurrentQueueExtensions.Clear(Variable.GMQ_BUSINESS);
            try
            {
                Task.Factory.StartNew(m =>
                {
                    ...
                    {
                        if (Variable.GMQ_BUSINESS.Count > 0)
                        {
                            dynamic data;
                            Variable.GMQ_BUSINESS.TryDequeue(out data);
        ...
        {
#if DEBUG
            XTrace.WriteLine("退出跑商模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("退出跑商模块 当前计数{0}", count));
