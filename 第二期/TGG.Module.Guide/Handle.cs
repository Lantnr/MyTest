...
namespace TGG.Module.Guide
{
    ...
    public class Handle
    {
        ...
        {
#if DEBUG
            XTrace.WriteLine("进入大命令模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("进入大命令模块 当前计数{0}", count));

            _token = new CancellationTokenSource();
            var _module = CommonHelper.GetCDTSTM(this.GetType().Namespace, count, true);
            ConcurrentQueueExtensions.Clear(Variable.GMQ_GUIDE);
            try
            {
                Task.Factory.StartNew(m =>
                {
                    ...
                    {
                        if (Variable.GMQ_GUIDE.Count > 0)
                        {
                            dynamic data;
                            Variable.GMQ_GUIDE.TryDequeue(out data);
        ...
        {
#if DEBUG
            XTrace.WriteLine("退出大命令模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("退出大命令模块 当前计数{0}", count));
