...
namespace TGG.Module.User
{
    ...
    public class Handle
    {
        ...
        {
#if DEBUG
            XTrace.WriteLine("进入用户模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("进入用户模块 当前计数{0}", count));

            _token = new CancellationTokenSource();
            var _module = CommonHelper.GetCDTSTM(this.GetType().Namespace, count, true);
            ConcurrentQueueExtensions.Clear(Variable.GMQ_USER);
            try
            {
                Task.Factory.StartNew(m =>
                {
                    ...
                    {
                        if (Variable.GMQ_USER.Count > 0)
                        {
                            dynamic data;
                            Variable.GMQ_USER.TryDequeue(out data);
        ...
        {
#if DEBUG
            XTrace.WriteLine("退出用户模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("退出用户模块 当前计数{0}", count));
