...
namespace TGG.Module.User.Service
{
    ...
    public partial class Common
    {
        ...
        public Player GetPlayer(tg_user user)
        {
            ...
            if (!daminglog.Any()) { daminglog = InitDamingLog(user.id); }
        ...
        public tg_user_extend InitUserExtend(tg_user user)
        {
            ...
            if (_b == null || _t == null || _s == null) return new tg_user_extend();
            var base_bag = Convert.ToInt32(_b.value);
            var base_train_bar = Convert.ToInt32(_t.value);
            var shot_count = Convert.ToInt32(_s.value);

            var model = new tg_user_extend { user_id = user.id, bag_count = base_bag, train_bar = base_train_bar, shot_count = shot_count };
            model.Save();
            return model;
        }
