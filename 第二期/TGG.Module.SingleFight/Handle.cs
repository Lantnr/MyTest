...
namespace TGG.Module.SingleFight
{
    /// <summary>
    /// 模块处理类
    /// </summary>
    public class Handle
    {
        ...
        {
#if DEBUG
            XTrace.WriteLine("进入一将讨模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("进入一将讨模块 当前计数{0}", count));

            _token = new CancellationTokenSource();
            var _module = CommonHelper.GetCDTSTM(this.GetType().Namespace, count, true);
            ConcurrentQueueExtensions.Clear(Variable.GMQ_SINGLEFIGHT);
            try
            {
                Task.Factory.StartNew(m =>
                {
                    ...
                    {
                        if (Variable.GMQ_SINGLEFIGHT.Count > 0)
                        {
                            dynamic data;
                            Variable.GMQ_SINGLEFIGHT.TryDequeue(out data);
        ...
        {
#if DEBUG
            XTrace.WriteLine("退出一将讨模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("退出一将讨模块 当前计数{0}", count));
