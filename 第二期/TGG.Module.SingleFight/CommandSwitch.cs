...
using TGG.Module.SingleFight.Service;
...
namespace TGG.Module.SingleFight
{
    ...
    public class CommandSwitch
    {
        ...
        {
            return Switch((int)ModuleNumber.SINGLEFIGHT, commandNumber, session, data);
        }
        ...
        public ASObject Switch(int moduleNumber, int commandNumber, TGGSession session, ASObject data)
        {
...
#endif
            if (!CommonHelper.IsOpen(session.Player.Role.Kind.role_level, (int)OpenModelType.一将讨))
            ...
            {
                case (int)SingleFightCommand.SINGLE_FIGHT_NPC: { aso = SINGLE_FIGHT_NPC.GetInstance().CommandStart(session, data); break; }
