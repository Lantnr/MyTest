...
namespace TGG.Module.Friend.Service
{
    ...
    {
        private static FRIEND_ADD ObjInstance;

        /// <summary>FRIEND_ADD单体模式</summary>
        public static FRIEND_ADD GetInstance()
        {
            return ObjInstance ?? (ObjInstance = new FRIEND_ADD());
        }

        /// <summary>添加好友指令</summary>
        public ASObject CommandStart(TGGSession session, ASObject data)
        {
#if DEBUG
            XTrace.WriteLine("{0}:{1}", "FRIEND_ADD", "添加好友指令");
#endif

            var name = data.FirstOrDefault(q => q.Key == "name").Value.ToString();
            var friend = tg_user.GetUserByName(name, session.Player.User.id);
            //var friend = tg_user.GetFindByName(name);
            ...
            var rule = Variable.BASE_RULE.FirstOrDefault(m => m.id == "19001");
            ...
            else if (entity.friend_state != (int)FriendStateType.BLACKLIST)
                return ResultError((int)ResultType.FRIEND_EXIST); 
#if DEBUG
            XTrace.WriteLine("id  {0} user_id {1} friend_id {2} friend_state {3}", entity.id, entity.user_id, entity.friend_id, entity.friend_state);
#endif
            entity.friend_state = (int)FriendStateType.FRIEND;
#if DEBUG
