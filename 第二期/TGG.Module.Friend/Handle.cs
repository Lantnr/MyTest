...
namespace TGG.Module.Friend
{
    ...
    public class Handle
    {
        ...
        {
#if DEBUG
            XTrace.WriteLine("进入好友模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("进入好友模块 当前计数{0}", count));

            _token = new CancellationTokenSource();
            var _module = CommonHelper.GetCDTSTM(this.GetType().Namespace, count, true);
            ConcurrentQueueExtensions.Clear(Variable.GMQ_FRIEND);
            try
            {
                Task.Factory.StartNew(m =>
                {
                    ...
                    {
                        if (Variable.GMQ_FRIEND.Count > 0)
                        {
                            dynamic data;
                            Variable.GMQ_FRIEND.TryDequeue(out data);
        ...
        {
#if DEBUG
            XTrace.WriteLine("退出好友模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("退出好友模块 当前计数{0}", count));
