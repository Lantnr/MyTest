...
namespace TGG.Module.Notice
{
    ...
    public class Handle
    {
        ...
        {
#if DEBUG
            XTrace.WriteLine("进入公告模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("进入公告模块 当前计数{0}", count));

            _token = new CancellationTokenSource();
            var _module = CommonHelper.GetCDTSTM(this.GetType().Namespace, count, true);
            ConcurrentQueueExtensions.Clear(Variable.GMQ_NOTICE);
            try
            {
                Task.Factory.StartNew(m =>
                {
                    ...
                    {
                        if (Variable.GMQ_NOTICE.Count > 0)
                        {
                            dynamic data;
                            Variable.GMQ_NOTICE.TryDequeue(out data);
        ...
        {
#if DEBUG
            XTrace.WriteLine("退出公告模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("退出公告模块 当前计数{0}", count));
