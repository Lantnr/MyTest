...
namespace TGG.Module.Games
{
    ...
    public class Handle
    {
        ...
        {
#if DEBUG
            XTrace.WriteLine("进入游戏集模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("进入游戏集模块 当前计数{0}", count));

            _token = new CancellationTokenSource();
            var _module = CommonHelper.GetCDTSTM(this.GetType().Namespace, count, true);
            ConcurrentQueueExtensions.Clear(Variable.GMQ_GAMES);
            try
            {
                Task.Factory.StartNew(m =>
                {
                    ...
                    {
                        if (Variable.GMQ_GAMES.Count > 0)
                        {
                            dynamic data;
                            Variable.GMQ_GAMES.TryDequeue(out data);
        ...
        {
#if DEBUG
            XTrace.WriteLine("退出游戏集模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("退出游戏集模块 当前计数{0}", count));
