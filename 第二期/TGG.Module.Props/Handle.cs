...
namespace TGG.Module.Props
{
    ...
    public class Handle
    {
        ...
        {
#if DEBUG
            XTrace.WriteLine("进入背包模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("进入背包模块 当前计数{0}", count));

            _token = new CancellationTokenSource();
            var _module = CommonHelper.GetCDTSTM(this.GetType().Namespace, count, true);
            ConcurrentQueueExtensions.Clear(Variable.GMQ_PROP);
            try
            {
                Task.Factory.StartNew(m =>
                {
                    ...
                    {
                        if (Variable.GMQ_PROP.Count > 0)
                        {
                            dynamic data;
                            Variable.GMQ_PROP.TryDequeue(out data);
        ...
        {
#if DEBUG
            XTrace.WriteLine("退出背包模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("退出背包模块 当前计数{0}", count));
