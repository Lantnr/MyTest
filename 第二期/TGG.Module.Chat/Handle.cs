...
namespace TGG.Module.Chat
{
    ...
    public class Handle
    {
        ...
        {
#if DEBUG
            XTrace.WriteLine("进入聊天模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("进入聊天模块 当前计数{0}", count));

            _token = new CancellationTokenSource();
            var _module = CommonHelper.GetCDTSTM(this.GetType().Namespace, count, true);
            ConcurrentQueueExtensions.Clear(Variable.GMQ_CHAT);
            try
            {
                Task.Factory.StartNew(m =>
                {
                    ...
                    {
                        if (Variable.GMQ_CHAT.Count > 0)
                        {
                            dynamic data;
                            Variable.GMQ_CHAT.TryDequeue(out data);
        ...
        {
#if DEBUG
            XTrace.WriteLine("退出聊天模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("退出聊天模块 当前计数{0}", count));
