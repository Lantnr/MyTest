...
namespace TGG.Module.Siege
{
    ...
    public class Handle
    {
        ...
        {
#if DEBUG
            XTrace.WriteLine("进入美浓活动模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("进入美浓活动模块 当前计数{0}", count));

            _token = new CancellationTokenSource();
            var _module = CommonHelper.GetCDTSTM(this.GetType().Namespace, count, true);
            ConcurrentQueueExtensions.Clear(Variable.GMQ_SIEGE);
            try
            {
                Task.Factory.StartNew(m =>
                {
                    ...
                    {
                        if (Variable.GMQ_SIEGE.Count > 0)
                        {
                            dynamic data;
                            Variable.GMQ_SIEGE.TryDequeue(out data);
        ...
        {
#if DEBUG
            XTrace.WriteLine("退出美浓活动模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("退出美浓活动模块 当前计数{0}", count));
