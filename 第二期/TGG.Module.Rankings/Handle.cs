...
namespace TGG.Module.Rankings
{
    ...
    public class Handle
    {
        ...
        {
#if DEBUG
            XTrace.WriteLine("进入排名模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("进入排名模块 当前计数{0}", count));

            _token = new CancellationTokenSource();
            var _module = CommonHelper.GetCDTSTM(this.GetType().Namespace, count, true);
            ConcurrentQueueExtensions.Clear(Variable.GMQ_RANKINGS);
            try
            {
                Task.Factory.StartNew(m =>
                {
                    ...
                    {
                        if (Variable.GMQ_RANKINGS.Count > 0)
                        {
                            dynamic data;
                            Variable.GMQ_RANKINGS.TryDequeue(out data);
        ...
        {
#if DEBUG
            XTrace.WriteLine("退出排名模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("退出排名模块 当前计数{0}", count));
