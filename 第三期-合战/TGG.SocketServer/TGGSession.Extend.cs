...
namespace TGG.SocketServer
{
    ...
    public partial class TGGSession
    {
        ...
        /// <summary>是否执行了关闭方法</summary>
        private bool IsCloseExtend = false;
        ...
        private void RemoveUser()
        {
            try
            {
                ...
                if (!b)//再次尝试移除
                {
                    Variable.OnlinePlayer.TryRemove(Player.User.id, out dy);
                }
                var c = Variable.WarInUser.TryRemove(Player.User.id, out mid);
#if DEBUG
        ...
        private void SceneBackBorn(view_scene_user scene)
        {
            ...
            if (basebornpoint == null) return;
            if (!basebornpoint.coorPoint.Contains(',')) return;
        ...
        private void CheckDay(Int64 userid, int gt)
        {
            ...
            var rule = Variable.BASE_YOUYIYUAN.FirstOrDefault(m => m.type == gt);
            if (rule == null) return;
            ...
            switch (gt)
            {
                case (int)GameEnterType.花月茶道:
                    if (ex.tea_count < total)
                    {
                        ex.tea_count++;
                    }
                    break;
                case (int)GameEnterType.辩驳游戏:
                    if (ex.eloquence_count < total)
                    {
                        ex.eloquence_count++;
                    }
                    break;
                case (int)GameEnterType.老虎机:
                    if (ex.calculate_count < total)
                    {
                        ex.calculate_count++;
                    }
                    break;
                case (int)GameEnterType.猜宝游戏:
                    if (ex.ninjutsu_count < total)
                    {
                        ex.ninjutsu_count++;
                    }
                    break;
                case (int)GameEnterType.猎魂:
                    if (ex.ball_count >= total)
                    {
                        ex.ball_count++;
                    }
                    break;
