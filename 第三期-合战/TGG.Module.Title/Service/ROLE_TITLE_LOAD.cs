...
namespace TGG.Module.Title.Service
{
    ...
    /// 称号装备
    /// </summary>
    public class ROLE_TITLE_LOAD : IDisposable
    {
        ...
        public ASObject CommandStart(TGGSession session, ASObject data)
        {
            try
            {
...
#endif
                if (!data.ContainsKey("titleId") || !data.ContainsKey("roleId")) return null;     //验证前端数据
                ...
                if (title.user_id != session.Player.User.id || role.user_id != session.Player.User.id) return null;    //验证玩家数据，防止发包
        ...
        private ASObject CheckRole(TGGSession session, tg_role role, BaseRoleTitle btitle, tg_role_title title)
        {
            ...
            if (otid != 0) { role = CheckRoleInfo(role, (int)RoleDatatype.ROLEDATA_LOSE, otid); }  //更新武将属性信息 
        ...
        {
            var title = tg_role_title.GetTitleByTid(oid, role.user_id);   //查询被替换的称号信息
            ...
            return string.IsNullOrEmpty(btitle.attAddition) ?
                role :
                Common.GetInstance().UpdateRole(role, type, btitle.attAddition);    //称号有加成属性的，减去武将对应属性信息
        }
        ...
        private void ClearRole(tg_role_title title, Int64 rid)
        {
            ...
            if (Common.GetInstance().Count(title) == 0)    //验证判断卸载后的称号状态信息
