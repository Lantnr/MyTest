...
namespace TGG.Module.Work
{
    ...
    public class Handle
    {
        ...
        {
#if DEBUG
            XTrace.WriteLine("进入工作系统模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("进入工作系统模块 当前计数{0}", count));

            _token = new CancellationTokenSource();
            var _module = CommonHelper.GetCDTSTM(this.GetType().Namespace, count, true);
            ConcurrentQueueExtensions.Clear(Variable.GMQ_WORK);
            try
            {
                Task.Factory.StartNew(m =>
                {
                    ...
                    {
                        if (Variable.GMQ_WORK.Count > 0)
                        {
                            dynamic data;
                            Variable.GMQ_WORK.TryDequeue(out data);
        ...
        {
#if DEBUG
            XTrace.WriteLine("退出工作系统模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("退出工作系统模块 当前计数{0}", count));
