...
namespace TGG.Module.User.Service
{
    ...
    /// 创建玩家
    /// </summary>
    public class USER_CREATE : IDisposable
    {
        ...
        public ASObject CommandStart(TGGSession session, ASObject data)
        {
            ...
            try
            {
                if (!data.ContainsKey("isAdult") || !data.ContainsKey("userName") || !data.ContainsKey("camp") || !data.ContainsKey("influence") || !data.ContainsKey("vocation") || !data.ContainsKey("sex") || !data.ContainsKey("playerName")) return null;
                ...
                if (Util.IsFilterWords(userName)) return CommonHelper.ErrorResult((int)ResultType.DATA_WORDS_ERROR);
                ...
                var boffice = Variable.BASE_RULE.FirstOrDefault(m => m.id == "1013");
                var rbp = Variable.BASE_RULE.FirstOrDefault(q => q.id == "1001");
                if (rbp == null || boffice == null) return ResultError((int)ResultType.BASE_TABLE_ERROR);
                var birthplace = int.Parse(rbp.value); //读取角色出生地
                var office = Convert.ToInt32(boffice.value);//读取默认官职
                ...
                var gold = 0;    //默认注册账号元宝
                var rule_gold = Variable.BASE_RULE.FirstOrDefault(q => q.id == "1014");
                if (rule_gold != null) gold = Convert.ToInt32(rule_gold.value);

                var user = tg_user.InitUser(userName, roleid, Convert.ToInt16(sex), playerName, birthplace, player_vocation,
                    player_influence, player_camp, birthplace, Convert.ToInt32(xy[0]), Convert.ToInt32(xy[1]), gold, office);
                if (user == null) return null;
                var rs = RecordSeesion(session, user, isAdult);
                if (!rs) return null;
                InsertGame(user.id);
                report_record_login.Login(user.id);
