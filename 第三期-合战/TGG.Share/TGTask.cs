...
namespace TGG.Share
{
    ...
    /// 任务
    /// </summary>
    public class TGTask : IDisposable
    {
        public void Dispose()
        {
        ...
        private string GetBusinessStepTask(Int64 user_id)
        {
            ...
            //获取身份
            var ide = 1;
            if (Variable.OnlinePlayer.Keys.Contains(user_id))
            {
                var s = Variable.OnlinePlayer[user_id] as TGGSession;
                var _ide_id = s.Player.Role.Kind.role_identity;
                var base_ide = Variable.BASE_IDENTITY.FirstOrDefault(m => m.id == _ide_id);
                ide = base_ide.value;
            }
            ...
            //var comm=Variable.BASE_GOODS.Where(m=>m.identity<=ide &&target.Contains(m.id));
            ...
            var goods = RandomGoods(list_random, ide);
        ...
        /// <summary>随机一个自己商圈的非目的地町的货物</summary>      
        Int32 RandomGoods(List<Int32> list_random, Int32 ide)
        {
            var index = RNG.Next(0, list_random.Count() - 1);
            var goods = list_random[index];
            var be = Variable.BASE_GOODS.FirstOrDefault(m => m.id == goods);
            if (be.identity <= ide) return goods;
            return RandomGoods(list_random, ide);
        }
        ...
        /// <summary> 根据任务概率判断任务概率 </summary>
        public int GetTaskProb(RoleItem role, int type)
        {
            var baselist = Variable.BASE_TASK_PROB.Where(q => q.type == type).ToList();
            if (!baselist.Any()) return 0;
            var first = baselist.First();
            var myvalue = GetLifeSkillType(role, first.skillOrAtt);
            var basedata = baselist.FirstOrDefault(q => q.value >= myvalue);
            ...
            if (basedata == null || basedata2 == null) return 0;
            return basedata.prob + basedata2.prob2;      //总的概率值
        }
        ...
        /// <summary>随机获取新的任务</summary>
        public List<tg_task> RandomTask(Int64 userid, int type)
        {
            var mytask = new List<tg_task>();
            var list = Variable.BASE_APPRAISE.Where(m => m.type == type).ToList();
            var basedata = Variable.BASE_RULE.FirstOrDefault(q => q.id == "21001");
            if (basedata == null) return mytask;
            var basecount = Convert.ToInt32(basedata.value);
            if (list.Count < basecount) return mytask;
            var indexlist = RNG.Next(0, list.Count - 1, basecount);
            mytask.AddRange(indexlist.Select(item => BuildNewRoleTask(list[item].id, userid, type)));
            return mytask;
        }
        ...
        public tg_task BuildNewRoleTask(int baseid, Int64 userid)
        {
            return new tg_task()
            {
                ...
                user_id = userid,
                task_step_data = string.Empty
            };

        }

        /// <summary> 组装新的家臣任务 </summary>
        public tg_task BuildNewRoleTask(int baseid, Int64 userid, int type)
        {
            return new tg_task()
            {
                task_id = baseid,
        ...
        public Tuple<int, string> CheckTaskStep(string basestep, string mystep, Int64 userid, int npc)
        {
            ...
            var i = GetDifferentStep(mysteplist, basesteplist);
            if (i < 0)
            {
                return Tuple.Create((int)ResultType.TASK_STEP_NOT_EXCEPT, "");
            }
            var onestep = mysteplist[i];
            var baseonestep = basesteplist[i];
        ...
        private int GetDifferentStep(List<TaskStep> steps, List<TaskStep> basestep)
        {
            for (int i = 0; i < steps.Count; i++)
            {
                if (steps[i].BaseId == basestep[i].BaseId && steps[i].Type == basestep[i].Type &&
                    steps[i].FinishValue == basestep[i].FinishValue && steps[i].FinishValue1 == basestep[i].FinishValue1) continue;
                return i;
            }
            return -1;
        }
        ...
        {
            var sf = new Share.Fight.Fight();
            var fight = sf.GeFight(userid, baseid, fighttype, 0, false, true);
            sf.Dispose();
        ...
        public void MainTaskUpdate(TaskStepType type, Int64 userid, Int32 baseid, Int32 count)
        {
            ...
            {
                XTrace.WriteLine("主线任务更新出错{0}", userid);
                return;
        ...
        #region 任务推送  (主要用于大名令、评定主线任务结束后推送)
        /// <summary>大名令、评定主线任务结束后推送</summary>
        /// <param name="user_id"></param>
        /// <param name="type">0:大名令主线任务  1;评定主线任务</param>
        public void TaskDaMingPush(Int64 user_id, int type)
        {
            var task = tg_task.GetEntityByType(user_id);
            if (task == null) return;
            var rule = new BaseRule();
            switch (type)
            {
                case 0: rule = Variable.BASE_RULE.FirstOrDefault(m => m.id == "2018"); break;
                case 1: rule = Variable.BASE_RULE.FirstOrDefault(m => m.id == "2019"); break;
            }
            if (rule == null) return;
            if (task.task_id == Convert.ToInt32(rule.value))
                GetNewTask(task);
        }

        /// <summary>获取新的主线任务并推送</summary>
        public tg_task GetNewTask(tg_task task)
        {
            var basetask = Variable.BASE_TASKMAIN.FirstOrDefault(q => q.id == task.task_id); //基表数据
            ...
            var role = tg_role.GetEntityByUserId(task.user_id);
            if (role == null)
                return null;
            var newlevle = role.role_level;
            if (newlevle < Variable.BASE_TASKMAIN[baseindex + 1].openLevel)  //等级验证 
            {
        ...
        /// <summary>推送主线任务</summary>
        public void TaskPush(tg_task newtask)
        {
            ...
            var dic = new Dictionary<string, object>() { { "0", EntityToVo.ToTaskVo(newtask) } };
            var pv = session.InitProtocol((int)ModuleNumber.TASK,
                (int)TaskCommand.TASK_PUSH, (int)ResponseType.TYPE_SUCCESS, new ASObject(dic));
            session.SendData(pv);
        }
        ...
        public tg_task BuildNewTask(Int32 task_id, string step, long id, Int64 userid)
        {
            var task = new tg_task()
            {
                ...
                id = id,
                task_id = task_id,
                task_step_data = GetInsertValue(step),
                task_state = (int)TaskStateType.TYPE_UNRECEIVED,
                user_id = userid
            };
            return task;
        ...
        public string GetInsertValue(string value)
        {
            ...
            foreach (var item in arr)  //对每个任务步骤处理
            {
                ...
                if (steptype == (int)TaskStepType.STUDY_SKILL || steptype == (int)TaskStepType.TRAIN) //有次数要求的npc
                    type[1] = "0";
                if (steptype == (int)TaskStepType.VOCATION_COUNT)
                {
                    type[1] = "0";
                    type[2] = "0";
                }
        ...
        /// <summary>统计评定和高职评定任务次数</summary>
        /// <param name="userid"></param>
        /// <param name="type">评定任务类型</param>
        public void VocationAdd(Int64 userid, int type)
        {
            var task = tg_task.GetEntityByType(userid);
            var rule = Variable.BASE_RULE.FirstOrDefault(m => m.id == "2019");
            if (task == null || rule == null) return;
            if (task.task_id != Convert.ToInt32(rule.value)) return;

            var newvalue = "";
            if (task.task_step_data == null) return;
            var values = task.task_step_data.Split('_').ToList();

            if (type == 1)
            {
                var i = Convert.ToInt32(values[1]);
                if (i < 3)
                {
                    i++;
                    values[1] = Convert.ToString(i);
                }
            }
            else
            {
                var i = Convert.ToInt32(values[2]);
                if (i == 0)
                {
                    i++;
                    values[2] = Convert.ToString(i);
                }
            }
            newvalue += string.Join("_", values);
            task.task_step_data = newvalue;

            task.Update();
            var basetask = Variable.BASE_TASKMAIN.FirstOrDefault(m => m.id == task.task_id);
            if (basetask != null)
            {
                if (task.task_step_data == basetask.stepCondition)
                    TaskDaMingPush(userid, 1);
            }
        }
        ...
        /// <summary>重置躲猫猫任务</summary>
        public void ResetDuoMaoMaoTask()
        {
            var list = tg_task.GetEntityByTaskState();
            if (list.Any())
            {
                var basetask = Variable.BASE_TASKVOCATION.FirstOrDefault(m => m.id == list[0].task_id);
                if (basetask == null) return;
                foreach (var item in list)
                {
                    item.task_state = (int)TaskStateType.TYPE_UNRECEIVED;
                    item.task_step_data = basetask.stepInit;
                    item.Save();
                }
            }
        }
        ...
        {
            var list = tg_task.GetEntityByType();
            var rule = Variable.BASE_RULE.FirstOrDefault(m => m.id == "2019");
            if (rule == null) return;
            var lists = list.Where(m => m.task_id == Convert.ToInt32(rule.value)).ToList();
            if (lists.Any())
            {
                foreach (var item in lists)
                {
                    if (item.task_step_data == null) continue;
                    var values = item.task_step_data.Split('_').ToList();

                    if (Convert.ToInt32(values[1]) >= 3 && Convert.ToInt32(values[2]) >= 1)
                    {
                        GetNewTask(item);
                    }
                }
        ...
        /// <summary> 组装新主线任务的数据 </summary>
        public tg_task BuildNewMainTask(Int32 taskId, string step, long id, Int64 userid)
        {
            var initstep = "";
            var state = (int)TaskStateType.TYPE_UNRECEIVED;

            var rule = Variable.BASE_RULE.FirstOrDefault(m => m.id == "2018"); //大名令主线任务
            var rule1 = Variable.BASE_RULE.FirstOrDefault(m => m.id == "2019"); //评定主线任务
            if (rule == null || rule1 == null) return null;
            if (taskId == Convert.ToInt32(rule.value) || taskId == Convert.ToInt32(rule1.value))
            {
                state = (int)TaskStateType.TYPE_UNFINISHED;
            }
            if (taskId != Convert.ToInt32(rule.value))
            {
                initstep = GetInsertValue(step);
            }
            var task = new tg_task()
            {
                task_type = (int)TaskType.MAIN_TASK,
