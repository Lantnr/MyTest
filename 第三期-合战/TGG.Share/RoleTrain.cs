    ...
    public class RoleTrain : IDisposable
    {
        ...
        private void RoleAttrituteLog(Int64 userid, int type, double add, double ov, double nv, int ModuleNumber, int command)
        {
            try
            {
                ...
                //(new Share.Log()).WriteLog(userid, (int)LogType.Get, ModuleNumber, command, "武将修行", "修行结束推送", "武将属性", 0,
                //    0, 0, logdata);
            }
            catch (Exception ex)
            {
        ...
        public void RoleTrainPush(Int64 user_id, Int64 rid)
        {
            ...
            var role = tg_role.GetRoleByIdAndUser(rid, user_id);
            if (role == null) return;
        ...
        /// <summary>获取武将修行指令字符串</summary>
        /// <param name="command"></param>
        /// <returns></returns>
        private string GetModuleString(int command)
        {
            switch (command)
            {
