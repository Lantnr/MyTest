...
namespace TGG.Module.Task.Service
{
    ...
    /// 职业任务刷新
    /// </summary>
    public class TASK_VOCATION_REFRESH : IDisposable
    {
        ...
        ~TASK_VOCATION_REFRESH()
        {
            Dispose();
        }
    
        #endregion
        //private static TASK_VOCATION_REFRESH _objInstance;

        //public static TASK_VOCATION_REFRESH getInstance()
        //{
        //    return _objInstance ?? (_objInstance = new TASK_VOCATION_REFRESH());
        //}
        ...
        private ASObject BuildData(int result, List<tg_task> dailytask)
        {
            ...
            {
                {"result", result},
                {"vocationTask",dailytask==null?null: Common.getInstance().ConvertListASObject(dailytask, "VocationTask")}
            };
        ...
        private tg_task GetNewTask(tg_task task, List<tg_task> newtasks)
        {
            ...
            return indexlist >= list.Count ? null : Common.getInstance().BuildNewVocationTask(step, taskid, userid, taskidentify, steptype);
        }
