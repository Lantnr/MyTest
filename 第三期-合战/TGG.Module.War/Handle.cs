...
namespace TGG.Module.War
{
    ...
    public class Handle
    {
        ...
        {
#if DEBUG
            XTrace.WriteLine("进入合战模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("进入合战模块 当前计数{0}", count));

            _token = new CancellationTokenSource();
            var _module = CommonHelper.GetCDTSTM(this.GetType().Namespace, count, true);
            ConcurrentQueueExtensions.Clear(Variable.GMQ_WAR);
            try
            {
                Task.Factory.StartNew(m =>
                {
                    ...
                    {
                        if (Variable.GMQ_WAR.Count > 0)
                        {
                            dynamic data;
                            Variable.GMQ_WAR.TryDequeue(out data);
        ...
        {
#if DEBUG
            XTrace.WriteLine("退出合战模块 当前计数{0}", count);
#endif
            DisplayGlobal.log.Write(string.Format("退出合战模块 当前计数{0}", count));
