...
namespace TGGUnitTest
{
    ...
    public class UnitTest
    {
        ...
        public void TestMethod1()
        {
            ...
            const int userId = 978;
            var user = tg_user.FindByid(userId);
            var ex = tg_user_extend.FindAllByName("user_id", userId, null, 0, 0);
            var daming = tg_daming_log.FindAllByName("user_id", userId, null, 0, 0);
            ...
            session.Player.Bag = new BagItem() { BagIsFull = false, Surplus = 20 };
            //session.Player.War.PlayerInCityId = 1000836;
            session.Player.War.Status = 1;
            ...
            //TGG.Module.Fusion.Service.FUSION_ENTER.GetInstance().CommandStart(session, data);
            //TGG.Module.Fusion.Service.FUSION_START.GetInstance().CommandStart(session, new ASObject(new Dictionary<string, object> { { "type", 1 }, { "id", 367 } }));
            //TGG.Module.Fusion.Service.FUSION_ACCELERATE.GetInstance().CommandStart(session, data);
            //TGG.Module.Fusion.Service.FUSION_PACKET_BUY.GetInstance().CommandStart(session, data);
            ...
            //var data = new ASObject(new Dictionary<string, object> { { "type", 6 }, { "count", 100000 } });
            //var data = new ASObject(new Dictionary<string, object> { { "type", (int)WarRoleStateType.LEVY }, { "id", 23 }, { "count", 1 } });
            //TGG.Module.War.Service.SkyCity.WAR_SKYCITY_ENTER.GetInstance().CommandStart(session, data);
            //TGG.Module.War.Service.SkyCity.WAR_SKYCITY_QUIT.GetInstance().CommandStart(session, data);
            //TGG.Module.War.Service.SkyCity.WAR_SKYCITY_UNLOCK.GetInstance().CommandStart(session, data);


            //TGG.Module.War.Service.SkyCity.WAR_SKYCITY_START.GetInstance().CommandStart(session, data);
            //TGG.Module.War.Service.SkyCity.WAR_SKYCITY_ACCELERATE.GetInstance().CommandStart(session, data);


            #endregion

            //(new TGG.Share.ActivityOpenService()).PayAdd(user.id, 10000);

            var data = new ASObject(new Dictionary<string, object>() { { "type", 17 } });
            //TGG.Module.Role.Service.RECRUIT_COLLECT_ENTER.GetInstance().CommandStart(session, data);
            //TGG.Module.Role.Service.RECRUIT_COLLECT_REWARD.GetInstance().CommandStart(session, data);

            //TGG.Module.Rankings.Service.RANKING_WAR_DUPLICATE.GetInstance().CommandStart(session, data);
            var aso = new TGG.Module.Role.Service.SELECT_GENRE_ROLE();
            aso.CommandStart(session, data);
